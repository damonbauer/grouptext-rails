...
class EventRepliesJob < ApplicationJob
  ...
  def perform(message_id:, selected_list_id:, send_to:)
    ...
    Rails.logger.info "EventRepliesJob: message_id: #{message_id}, selected_list_id: #{selected_list_id}, send_to: #{send_to}"
    Rails.logger.info "EventRepliesJob: #{event_decision_reply_url}"
